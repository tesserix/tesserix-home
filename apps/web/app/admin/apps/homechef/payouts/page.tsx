...
function HomechefPayoutsInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/apps/homechef/payouts?${queryString()}`,
        {
          credentials: "include",
        },
      );
  ...
  async function markPaid(row: StatementRow) {
    ...
    try {
      const res = await fetch(
        `/api/admin/apps/homechef/payouts/${row.id}/mark-paid`,
        {
          method: "PUT",
          credentials: "include",
      ...
      window.alert(
        `Failed to mark paid: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
      setBusyId(null);
    }
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-4 p-6">
        <p className="text-sm text-muted-foreground">
          Weekly chef settlement statements. Calculation is automated.
          Disbursement is manual unless the Cashfree Payouts rail below is
          configured — then statements can be paid straight to the chef&apos;s
          bank account and settled here automatically.
        </p>
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-t border-border">
                    <td className="px-3 py-2">
                      {r.chef_name ?? r.chef_id.slice(0, 8)}
                    </td>
