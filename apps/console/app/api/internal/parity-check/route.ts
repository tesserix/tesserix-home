 ...
 *
 * Which pairs run, in what order, and what happens when one of them fails —
 * all of that is `runAllParityPairs` in `lib/billing/parity-run.ts`, along
 * with the invariants behind it (every pair gets a row; one pair's failure
 * costs no other pair its row; an unrecorded row is the one failure the design
 * cannot record). It lives there because the catalog surface's audited re-run
 * action runs the same cross product, and a second copy of that loop is a
 * second place for an early return to creep into the window's evidence.
 *
 * What stays here is the HTTP half: who may start a run, and how the two flags
 * that come back become 500 / 502 / 200.
 *
...
export async function POST(): Promise<NextResponse> {
  ...
  // Every pair, mode-major, one row each — see `runAllParityPairs` for why it
  // never gives up on the first failure.
  const { runs: recorded, unrecordable, checkFailed } = await runAllParityPairs();
  ...
  if (unrecordable) {
    ...
    // covers the gap — silence here would be the day-shaped hole
    // `runAllParityPairs` exists to prevent.
    //
  ...
  const runs: ParityRunBody[] = recorded.map((run) => ({
    mode: run.mode,
    source: run.source,
    outcome: run.outcome,
