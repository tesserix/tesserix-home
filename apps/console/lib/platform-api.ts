...
function unwrapEnvelope(label: string, status: number, body: unknown): Envelope {
  ...
  // The code is carried on the error as well as spelled into the message.
  // `emailtemplates` needs it structurally: `EXTERNAL_SERVICE_ERROR` and
  // `SERVICE_UNAVAILABLE` are both 503 and want different sentences, and
  // reading them back out of this string would be a parser over the platform
  // API's prose. "UNKNOWN" is not carried — an absent code and an
  // unrecognisable one are the same thing to a reader, and passing the
  // sentinel through would invite a `code === "UNKNOWN"` branch that means
  // nothing.
  throw new PlatformApiError(`${label}: ${code} — ${message}`, status, {
    code: code === "UNKNOWN" ? undefined : code,
  });
}
...
// ---- mark8ly email templates (#588) ---------------------------------------
//
// PLATFORM-API ONLY, with no apps/web fallback, and unlike the ticket
// functions above that is not merely because the console never had this
// surface. apps/web DOES serve it — over a direct connection to mark8ly's
// database plus an internal cache-eviction ping — and that transport is the
// thing this surface exists to stop using. The console holds no mark8ly
// credential and must not acquire one, so there is no path to fall back to.
//
// The four functions mirror the module's route table exactly
// (`emailtemplates/internal/handler/handler.go`). Both writes send an
// `idempotency-key`: the module REFUSES a write without one rather than
// minting its own, and `error-no-idempotency-key.json` is the golden for what
// omitting it looks like.

/** The whole registry, or one source's half of it. */
export async function fetchEmailTemplates(
  source?: string,
): Promise<import("./email-templates").EmailTemplatesPage> {
  // `?source=` only when it narrows. The module rejects unknown query
  // parameters and an empty `source=` is not the same request as no `source`.
  const suffix = source ? `?source=${encodeURIComponent(source)}` : "";
  const data = await platformRequest("email templates", `/v1/email-templates${suffix}`);
  return parseEmailTemplatesPage(data);
}

/**
 * One template, bodies included.
 *
 * For an UNAUTHORED key the bodies are mark8ly's embedded default rather than
 * empty strings — what is sending right now, which is what an editor has to
 * open with. `state` is what tells the two apart; the bodies do not.
 */
export async function fetchEmailTemplate(
  id: string,
): Promise<import("./email-templates").EmailTemplateDetail> {
  const data = await platformRequest(
    "email template",
    `/v1/email-templates/${encodeURIComponent(id)}`,
  );
  return parseEmailTemplateDetail(data);
}

/**
 * The write body, in mark8ly's own spelling so platform-api forwards it rather
 * than translating it.
 *
 * `variables` is carried back unchanged from the read, not composed here: the
 * declared interpolations belong to the product's Go call site, and a console
 * that invented them would be authoring a contract it does not own. `status`
 * is `draft` or `published` and is validated BY mark8ly — platform-api keeps
 * no copy of that vocabulary, and neither does this.
 */
export interface EmailTemplateUpsert {
  subject: string;
  html_body: string;
  text_body: string;
  variables: import("./email-templates").EmailTemplateVariable[];
...
export async function saveEmailTemplate(
  ...
  const data = await platformRequest(
    ...
    `/v1/email-templates/${encodeURIComponent(id)}`,
    {
      ...
      headers: {
        "content-type": "application/json",
        // Required, and refused rather than generated by the API — see the
        // block comment above.
        "idempotency-key": idempotencyKey(),
      },
      body: JSON.stringify(input),
...
/**
 * Send one REAL email through mark8ly's production send path.
 *
 * `vars` is sent even when empty: the module substitutes `{}` for a null map
 * before forwarding, so an omitted key and an empty object are the same
 * request — sending it explicitly keeps that true from this side too.
 */
export async function testSendEmailTemplate(
  ...
  await platformRequest(
    ...
    `/v1/email-templates/${encodeURIComponent(id)}/test-send`,
    {
      method: "POST",
      headers: { "content-type": "application/json", "idempotency-key": idempotencyKey() },
