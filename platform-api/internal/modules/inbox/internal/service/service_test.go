...
func svc(t *testing.T, bodies map[string]string) *Service {
	...
	for slug, body := range bodies {
		products = append(products, federation.Product{Slug: slug, Services: []federation.Service{{Name: slug, BaseURL: serving(t, body).URL, Secret: "s", Endpoints: []string{"inbox"}}}})
...
func TestEstateReportsAFailedProductWithoutLosingTheOthers(t *testing.T) {
	...
	fed := federation.NewClient(federation.NewRegistry([]federation.Product{
		{Slug: "kora", Services: []federation.Service{{Name: "kora", BaseURL: serving(t, koraQueue).URL, Secret: "s", Endpoints: []string{"inbox"}}}},
		{Slug: "other", Services: []federation.Service{{Name: "other", BaseURL: broken.URL, Secret: "s", Endpoints: []string{"inbox"}}}}}), nil)
