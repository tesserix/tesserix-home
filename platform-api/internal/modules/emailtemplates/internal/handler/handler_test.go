...
func serveWith(t *testing.T, upstream http.Handler, slugs []string, roles ...string) *api {
	...
	fed := federation.NewClient(federation.NewRegistry([]federation.Product{
		{Slug: productSlug, Services: []federation.Service{{Name: productSlug, BaseURL: srv.URL, Secret: "test-secret", Endpoints: []string{"email-templates"}}}}}), srv.Client())
...
func TestAConfiguredButUnreachableProductIs503AndNot501(t *testing.T) {
	...
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	fed := federation.NewClient(federation.NewRegistry([]federation.Product{
		{Slug: productSlug, Services: []federation.Service{{Name: productSlug, BaseURL: dead.URL, Secret: "s", Endpoints: []string{"email-templates"}}}}}), dead.Client())
	mux := http.NewServeMux()
