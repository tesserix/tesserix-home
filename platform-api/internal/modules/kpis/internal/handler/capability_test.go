...
// than passing untested. Mirrors audit, tenants, inbox and tools.
func TestEveryRouteNamesItsCapability(t *testing.T) {
	want := map[string]auth.Capability{
		...
		// a headline metric map is an operational view of a product, and
		// `billing` is reserved for the revenue surfaces §8.2 defines — it is
		// still marked RESERVED in platform-auth's capabilities.ts, and this
		// route is not the one that should first claim it.
		"GET /v1/kpis": auth.CapPlatform,
	}
	for _, r := range handler.RouteTable {
		...
		if capability != auth.CapPlatform {
			t.Errorf("route %s: capability %q; KPIs are an Operate read", key, capability)
		}
	}
}
