...
| **A local checkout of another repo** | On 2026-08-15 a Zitadel blocker was filed stating backups were disabled, quoting `charts/apps/zitadel-postgres/values.yaml`. The local `tesserix-k8s` checkout was **weeks behind** the deployed revision. On `main` backups are enabled, with a different bucket, a different service account and 3d rather than 30d retention — and they were **failing**. The filed issue was wrong in cause and in fix, while looking well-evidenced because it quoted a real file with a real line number. | `kubectl get application -n argocd <app> -o jsonpath='{.status.sync.revision}'`, then `git show <rev>:<path>` — read the revision the cluster is actually running, not the one on disk. |
| **CNPG `Cluster in healthy state`** | `zitadel-postgres` reported healthy with 3/3 instances while `ContinuousArchiving` had been `False` for nine hours and every WAL push was failing. Cluster health covers replication and readiness, **not** whether a backup exists. Replicas are not backups: three of them replicate a bad migration faithfully. | The `ContinuousArchiving` condition, and the `Backup` object's phase — `walArchivingFailing` is a terminal phase that does not self-heal into a good backup. |
